...
function App() {
  ...
  const { user, setUser: loginUser, logoutUser } = useAuth();
  const navigate = useNavigate();
  const hideNavbarFooter = ['/login', '/signup', '/login-success'].includes(location.pathname);
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        ...
        // loginUser({ ...user, ...data, role: data.role }); // Update user context
      } catch (error) {
    ...
    fetchProfile();
  }, [navigate, loginUser, logoutUser]); // Remove 'user' from deps if not needed
